...
console.log('🌐 TickerAll API URL:', TICKERALL_API_URL);
...
async function tickerallRequest(endpoint, method = 'GET', data = null) {
    try {
        ...
        console.log(`📡 TickerAll Request: ${method} ${TICKERALL_API_URL}${endpoint}`);
        const response = await axios(config);
        return response.data;
    } catch (error) {
        console.error('❌ TickerAll API error:', error.response?.data || error.message);
...
app.post('/api/set-exness-creds', authenticate, async (req, res) => {
    try {
        ...
        const session = await tickerallRequest('/sessions', 'POST', {
            broker: 'mt5',
            server: exnessServer,
        ...
        const accountInfo = await tickerallRequest(`/accounts/${session.id}`, 'GET');
        console.log(`💰 Balance: $${accountInfo.balance.toFixed(2)}`);
        
        const users = readUsers();
        users[req.user.email].tickerallSessionId = session.id;
...
async function getAISignal(symbol, accountId) {
    try {
        ...
        const volatility = Math.max(...prices.slice(-20)) - Math.min(...prices.slice(-20));
        ...
        } else if (volatility > 0.5 && rsi < 50) {
            action = 'BUY';
            confidence = 0.6;
            reasons.push(`High volatility with low RSI`);
        }
        ...
        } else if (volatility > 0.5 && rsi > 55) {
            action = 'SELL';
            confidence = 0.6;
            reasons.push(`High volatility with high RSI`);
        }
        
        if (action === 'HOLD') {
            if (ma20 > ma50 && momentum > 0) {
                action = 'BUY';
                confidence = 0.55;
        ...
        return { action: 'BUY', confidence: 0.5, reasons: ['API error, attempting buy'], currentPrice: 0 };
    }
}
        ...
        if (profitPercent > 0) {
            if (profitPercent >= 2) {
                ...
                if ((position.side === 'buy' && rsi > 65) || (position.side === 'sell' && rsi < 35)) {
                    ...
                    shouldClose = true;
                    reason = `Profit ${profitPercent.toFixed(2)}% with low momentum`;
                }
            }
...
class HalalTradingEngine {
    ...
    async executeTrade(symbol, side, signal) {
        ...
        try {
            ...
            let order;
            if (side === 'buy') {
                order = await tickerallRequest(`/orders`, 'POST', {
    ...
    async closePosition(position, profitPercent, currentPrice) {
        try {
            await tickerallRequest(`/orders/${position.orderId}`, 'DELETE');
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🕋 100% HALAL EXNESS TRADING BOT - HIGH SPEED EDITION`);
